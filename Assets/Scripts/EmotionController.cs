...
public class EmotionController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (logBlendshapeNames && faceRenderer != null)
        {
            var mesh = faceRenderer.sharedMesh;
            var names = new System.Text.StringBuilder("[EmotionController] Available blendshapes:\n");
            for (int i = 0; i < mesh.blendShapeCount; i++)
                names.AppendLine($"  {mesh.GetBlendShapeName(i)}");
